...
const App = () => {
  ...
  // jika jam kunjungan  pasien lebih  kecil dari jam mulai maka tampilkan bisa berkunjung dokter dan  jika jam kunjungan lebih besar dari jam selesai maka tidak bisa berkunjung
  const handleData = () => {
    console.log(new Date(jamKunjungan.$d).getHours());
    console.log(new Date(jamKunjungan.$d).getMinutes());
    const jam = new Date(jamKunjungan.$d).getHours();
    const menit = new Date(jamKunjungan.$d).getMinutes();
    const gabungan = jam * 100;

    const splitJadwal = selectJadwalPraktekDokter.split("-");
    const jamMulai = splitJadwal[0];
    console.log(jamMulai);
    const jamSelesai = splitJadwal[1];
    console.log(jamSelesai);
    if (gabungan >= jamMulai && gabungan <= jamSelesai) {
      alert("Jam kunjung Tersedia,Silahkan Berkunjung ");
    } else {
      alert("bahwa tidak ada jam berkunjung ");
    }
  };
  return (
    <>
